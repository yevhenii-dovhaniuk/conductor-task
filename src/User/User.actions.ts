...
export const selectUser = (userName: string) => async (dispatch: ThunkDispatch<any, any, any>) => {
    await dispatch(getUserDetails(userName));
    dispatch(push(`/user/${userName}`));
};

export const getUserDetails = (userName: string) => async (
...
export const selectRepo = (repoName: string) => (
    dispatch: ThunkDispatch<UserState, never, AnyAction>,
